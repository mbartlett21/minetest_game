    ...
    for b = 0, 1 do
        for c = 0, 1 do
            for d = 0, 1 do
                ...
                local groups = { dig_immediate = 2, mesecon = 3 }
                ...
                minetest.register_node(nodename, {
                        ...
                        walkable = true,
                        groups = groups,
                        drop = "mesecons:microcontroller 1",
                        selection_box = {
                            type = "fixed",
...
local conductor = "group:mesecon_conductor_craftable"

minetest.register_craft({
    output = "mesecons:microcontroller 2",
    recipe = {
        {silicon,   silicon,   conductor},
        {silicon,   silicon,   conductor},
        {conductor, conductor, ''},
    }
})
...
yc.parsecode = function(code, pos)
	...
	while true do
		...
		if command == "if" then
			local result
			result, endi = yc.command_if(code, endi, yc.merge_portstates(Lreal, Lvirtual), eeprom)
			if result == nil then return nil end
			if result == true then  -- nothing
...
yc.action_setports = function(pos, L, Lv)
	...
	if Lv.a ~= L.a then
		...
		if L.a == true then mesecons.receptor_on(pos, rules)
		else mesecons.receptor_off(pos, rules) end
	end
	if Lv.b ~= L.b then
		rules = microc_rules[name.."0010"]
		if L.b == true then mesecons.receptor_on(pos, rules)
		else mesecons.receptor_off(pos, rules) end
	end
	if Lv.c ~= L.c then
		rules = microc_rules[name.."0100"]
		if L.c == true then mesecons.receptor_on(pos, rules)
		else mesecons.receptor_off(pos, rules) end
	end
	if Lv.d ~= L.d then
		rules = microc_rules[name.."1000"]
		if L.d == true then mesecons.receptor_on(pos, rules)
		else mesecons.receptor_off(pos, rules) end
	end
end
