...
local receiver_get_rules = function (node)
	...
	elseif node.param2 == 3 then
		rules = mesecons.rules.rotate_right(mesecons.rules.rotate_right(rules))
	elseif node.param2 == 0 then
		rules = mesecons.rules.rotate_right(rules)
	end
	return rules
end

mesecons.register_node("mesecons_receiver:receiver", {
	drawtype = "nodebox",
	paramtype = "light",
	paramtype2 = "facedir",
...
mesecons.register_node("mesecons_receiver:receiver_up", {
	drawtype = "nodebox",
	paramtype = "light",
	paramtype2 = "facedir",
	is_ground_content = false,
...
mesecons.register_node("mesecons_receiver:receiver_down", {
	drawtype = "nodebox",
	paramtype = "light",
	paramtype2 = "facedir",
	is_ground_content = false,
...
function mesecons.receiver_get_pos_from_rcpt(pos, param2)
	...
	elseif dir.z == 1 then
		rules = mesecons.rules.rotate_right(rules)
	elseif dir.y == -1 then
		rules = mesecons.rules.rotate_up(rules)
		rcvtype = "mesecons_receiver:receiver_up_off"
	elseif dir.y == 1 then
		rules = mesecons.rules.rotate_down(rules)
...
function mesecons.receiver_place(rcpt_pos)
	local node = minetest.get_node(rcpt_pos)
	local pos, rcvtype = mesecons.receiver_get_pos_from_rcpt(rcpt_pos, node.param2)
	...
	if string.find(nn.name, "mesecons:wire_") ~= nil then
		...
		mesecons.on_placenode(pos, nn)
	end
end

function mesecons.receiver_remove(rcpt_pos, dugnode)
	local pos = mesecons.receiver_get_pos_from_rcpt(rcpt_pos, dugnode.param2)
	local nn = minetest.get_node(pos)
	if string.find(nn.name, "mesecons_receiver:receiver_") ~= nil then
		local node = {name = "mesecons:wire"}
		minetest.set_node(pos, node)
		mesecons.on_placenode(pos, node)
	end
end

minetest.register_on_placenode(function (pos, node)
	if minetest.get_item_group(node.name, "mesecon_needs_receiver") == 1 then
		mesecons.receiver_place(pos)
	end
end)

minetest.register_on_dignode(function(pos, node)
	if minetest.get_item_group(node.name, "mesecon_needs_receiver") == 1 then
		mesecons.receiver_remove(pos, node)
	end
end)
...
function mesecons.buttonlike_onrotate(pos, node)
	minetest.after(0, mesecons.receiver_remove, pos, node)
	minetest.after(0, mesecons.receiver_place, pos)
end
