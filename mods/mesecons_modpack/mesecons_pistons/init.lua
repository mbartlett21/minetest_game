...
local function piston_get_rules(node)
	...
	local rules = table.copy(mesecons.rules.default)
...
local piston_on = function(pos, node)
	...
	local success, stack, oldstack = mesecons.mvps.push(pusher_pos, dir, 15)
	if not success then
		return
	end
	...
	mesecons.mvps.process_stack(stack)
	mesecons.mvps.move_objects(pusher_pos, dir, oldstack)
end
...
local function piston_off(pos, node)
	...
	local success, stack, oldstack = mesecons.mvps.pull_single(pullpos, dir, 15)
	if success then
		mesecons.mvps.move_objects(pullpos, vector.multiply(dir, -1), oldstack, -1)
	end
end
...
local function piston_orientate(pos, placer)
	...
	minetest.after(0, mesecons.on_placenode, pos, node)
end
...
mesecons.mvps.fixed.register_node("mesecons_pistons:piston_pusher_normal", piston_pusher_get_stopper)
mesecons.mvps.fixed.register_node("mesecons_pistons:piston_pusher_sticky", piston_pusher_get_stopper)
...
mesecons.mvps.fixed.register_node("mesecons_pistons:piston_normal_on", piston_get_stopper)
mesecons.mvps.fixed.register_node("mesecons_pistons:piston_sticky_on", piston_get_stopper)
...
minetest.register_alias("mesecons:piston_sticky", "mesecons_postons:poston_sticky_off")
