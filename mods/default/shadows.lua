...
core.register_chatcommand("shadow_intensity", {
    ...
    description = "Set shadow intensity for the current world.",
    func = function(name, param)
        ...
        if new_intensity < 0 or new_intensity > 1 or new_intensity == nil then
            minetest.chat_send_player(name, minetest.colorize("#ff0000", "Invalid intensity."))
            return true
        end

        if new_intensity ~= default_intensity then
            minetest.chat_send_player(name, "Set intensity to " .. new_intensity .. ".")
            storage:set_float("intensity", new_intensity)
        else
            minetest.chat_send_player(name, "Set intensity to default value (" .. default_intensity .. ").")
