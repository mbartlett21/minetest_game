...
local function update_bookshelf(pos)
	...
	if n_written + n_empty == 0 then
        ...
        local s_written = n_written == 1 and "" or "s"
        local s_empty = n_empty == 1 and "" or "s"
        if n_written == 0 then n_written = "No" end
        if n_empty == 0 then n_empty = "no" end
		meta:set_string("infotext", "Bookshelf (" ..
            n_written .. " book" .. s_written .. " written, " ..
            n_empty   .. " book" .. s_empty   .. " empty)")
	end
end
...
minetest.register_node("books:bookshelf", {
	...
	groups = { choppy = 3, oddly_breakable_by_hand = 2, flammable = 3, creative = 1 },
	...
	on_metadata_inventory_move = function(pos, from_list, from_index, to_list, to_index, count, player)
		minetest.log("action", "[books] " .. player:get_player_name() ..
			" moves bookshelf inventory at " .. minetest.pos_to_string(pos))
		update_bookshelf(pos)
	end,
	on_metadata_inventory_put = function(pos, listname, index, stack, player)
		minetest.log("action", "[books] " .. player:get_player_name() ..
			" puts bookshelf inventory at " .. minetest.pos_to_string(pos))
		update_bookshelf(pos)
	end,
	on_metadata_inventory_take = function(pos, listname, index, stack, player)
		minetest.log("action", "[books] " .. player:get_player_name() ..
			" takes from bookshelf inventory at " .. minetest.pos_to_string(pos))
		update_bookshelf(pos)
	end,
...
minetest.register_craftitem("books:book", {
	...
	groups = {book = 1, flammable = 3, creative = 1},
	on_use = book_on_use,
})
